        ...
        db.run(
            ...
            function (err) {
                ...
                db.run(
                    `INSERT INTO stock_history (
                        item_id, type, quantity, previous_quantity,
                        new_quantity, note, person, expected_return_date, date, user_email
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [itemId, type, quantity, previousQuantity, newQuantity, note || null, person || null, expectedReturnDate || null, date, req.user.email],
                    function (err) {
                        if (err) {
                            res.status(500).json({ error: err.message });
                            return;
